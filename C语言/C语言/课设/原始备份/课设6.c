...
void weishu(struct jilu *xjilu,int x,int y)
{
	...
	for(i=0;i<10;i++)
	{
		...
		{
			case '+':zongfen+=jiafa(diyi[i],dier[i],shuru,i);
					xjilu->shuzi[i][2]=shuru[i];
					xjilu->shuzi[i][3]=diyi[i]+dier[i];
					system("pause");system("cls");break;
			case '-':zongfen+=jianfa(diyi[i],dier[i],shuru,i);
					xjilu->shuzi[i][2]=shuru[i];
					xjilu->shuzi[i][3]=diyi[i]-dier[i];
					system("pause");system("cls");break;
			case '*':zongfen+=chengfa(diyi[i],dier[i],shuru,i);
					xjilu->shuzi[i][2]=shuru[i];
					xjilu->shuzi[i][3]=diyi[i]*dier[i];
					system("pause");system("cls");break;
			case '/':zongfen+=chufa(diyi[i],dier[i],shuru,i);				//存在不整除问题
					xjilu->shuzi[i][2]=shuru[i];
					xjilu->shuzi[i][3]=diyi[i]/dier[i];
					system("pause");system("cls");break;
			case '%':zongfen+=quyu(diyi[i],dier[i],shuru,i);
					xjilu->shuzi[i][2]=shuru[i];
					xjilu->shuzi[i][3]=diyi[i]%dier[i];
					system("pause");system("cls");break;
		}
