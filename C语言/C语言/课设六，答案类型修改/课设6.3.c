...
void wuzhanghao(void)
{
	...
	while(1)
	{
		...
		switch(xz)
		{
			...
			case '4':system("cls");										//如果选择查看记录，把链表新建到外面的话会多建一个
					show(head,geshu);
					system("pause");system("cls");break;
			case '0':flag++;break;
		}
...
{
	int i,ysf,diyi[10],dier[10],zongfen=0,cuowu=0,p;
	double shuru[10];
	double d;
	...
	for(i=0;i<10;i++)
	{
		xjilu->shuzi[i][0]=diyi[i]=rand()%x;				//随机数对x(10,100,1000)取余，得到一位，两位，三位，的数
		...
		switch(yunsuanfu[ysf])
		{
			case '+':xjilu->shuzi[i][2]=jiafa(diyi[i],dier[i],shuru,i);
					...
					system("pause");system("cls");break;
			case '-':xjilu->shuzi[i][2]=jianfa(diyi[i],dier[i],shuru,i);
					zongfen+=xjilu->shuzi[i][2];
					xjilu->daan[i][0]=shuru[i];
					xjilu->daan[i][1]=diyi[i]-dier[i];
					system("pause");system("cls");break;
