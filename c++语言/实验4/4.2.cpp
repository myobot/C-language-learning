...
Person::Person():number(""),name(""),sex(0),age(0)
{
}
Person::Person(string Number="",string Name="",string Sex="",int Age=0):number(Number),name(Name),sex(Sex),age(Age)
{
	if(age<0)
	{
		cout <<"年龄不能为负！\n";
...
Student::Student():Person(),department(""),score(0)
{
}
Student::Student(string Number="",string Name="",string Sex="",int Age=0,string Department="",int Score=0):Person(Number,Name,Sex,Age),department(Department),score(Score)
{
	if(score<0)
	{
		cout <<"成绩不能为负！\n";
...
Teacher::Teacher():Person(),profession(""),major("")
{
}
Teacher::Teacher(string Number="",string Name="",string Sex="",int Age=0,string Profession="",string Major=""):Person(Number,Name,Sex,Age),profession(Profession),major(Major)
{
}
...
int main()
{
	Student l("008321","李宁","男",19,"计算机科学与技术",99),y;
	Teacher	c("007321","常淑慧","女",32,"教授","计算机"),w;
	y.sinput();
	w.tinput();
	y.soutput();
	w.toutput();
