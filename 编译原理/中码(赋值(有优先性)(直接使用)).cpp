...
struct produce * ExpandInitialize(void)
{
	...
	char c[MAXLENGTH];
	char fuhao[13][MAXSYMBOLLENGTH]={{"$"},{"S"},{"E"},{"T"},{"F"},{"id"},{"="},{"+"},{"-"},{"*"},{"("},{")"},{"#"}};

	char wenfa[8][MAXLENGTH]={{"S -> id = E"},{"E -> E + T"},{"T -> T * F"},{"E -> - E"},{"F -> ( E )"},{"F -> id"},{"E -> T"},{"T -> F"}};
...
void Analysis(struct produce *P, int ***analysistable)
{
	...
	printf("请输入要分析的串(每个单词用空格隔开，请不要超过%d个字符)\n",MAXSTRING);
	gets(str);
	fflush(stdin);
