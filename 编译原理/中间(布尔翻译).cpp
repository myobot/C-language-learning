...
------名称------布尔翻译
说明：
(只需要更改ProductionFunction(1262行)函数,及符号数组(42行),符号数(40行)，以对应不同文法)
------wzw---------------------
------------------------------*/
...
#define ERROR -1			//错误
#define symbolnum 13		//符号数
#define sentence 100

int ProductionFunction(FILE *fp,int Pnum,char idtable[][MAXSYMBOLLENGTH],int &nnumber,int &number,int p[3]);
static char symbol[symbolnum][6]={{"or"},{"and"},{"not"},{"true"},{"false"},{"("},{")"},{"=="},{"!="},{">"},{"<"},{">="},{"<="}};
struct produce
{
	int **produceR;		//产生式右部
	int *produceL;		//产生式左部
...
struct produce * ExpandInitialize(void)
{
	...
	char c[MAXLENGTH];
	char fuhao[14][MAXSYMBOLLENGTH]={{"$"},{"E"},{"T"},{"F"},{"or"},{"and"},{"not"},{"("},{")"},{"id"},{"rop"},{"true"},{"false"},{"#"}};
	char css[9][MAXLENGTH]={{"E -> E or T"},{"E -> T"},{"T -> T and F"},{"T -> F"},{"F -> not F"},{"E -> ( E )"},{"F -> id rop id"},{"E -> true"},{"E -> false"}};
...
int ropfind(char word[])
{
	int i;
	char rop[6][3]={{"=="},{"!="},{">"},{"<"},{">="},{"<="}};
	for(i=0;i<6;i++)
	{
		if(strcmp(word,rop[i])==0)
			return i;
	}
	return ERROR;
}
...
int gt[sentence],gtnum=0;
int judgegt[sentence];

char willprint[sentence][13+MAXSYMBOLLENGTH*2];
...
struct return4 ProductionFunction(int Pnum,char idtable[][MAXSYMBOLLENGTH],int &nextstat,struct return4 re[3],int rop)	//p符号位置
{
	struct return4 a;
	a.place=-1;
	switch(Pnum)
	{
		case 1: backpatch(re[1].False,re[0].Codebegin);
				a.Codebegin=re[1].Codebegin;
				a.True=merge(re[1].True,re[0].True);
				a.False=re[0].False;	
				return a;
				
		case 2:return re[0];

		case 3:	backpatch(re[1].True,re[0].Codebegin);
				a.Codebegin=re[1].Codebegin;
				a.True=re[0].True;
				a.False=merge(re[1].False,re[0].False);
				return a;

		case 4:	return re[0];

		case 5:	a.True=re[0].False;
				a.False=re[0].True;
				a.Codebegin=re[0].Codebegin;
				return a;

		case 6: return re[0];

		case 7:	a.True=nextstat;
				a.Codebegin=nextstat;
				a.False=nextstat+1;
				nextstat++;
				strcpy(willprint[gtnum],"if ");
				strcat(willprint[gtnum],idtable[re[1].place]);
				strcat(willprint[gtnum]," ");
				strcat(willprint[gtnum],symbol[rop]);
				strcat(willprint[gtnum]," ");
				strcat(willprint[gtnum],idtable[re[0].place]);
				strcat(willprint[gtnum]," goto ");
				judgegt[gtnum]=0;
				gt[gtnum++]=-1;
				nextstat++;
				strcpy(willprint[gtnum],"goto ");
				judgegt[gtnum]=0;
				gt[gtnum++]=-1;
				return a;

		case 8:	a.True=nextstat;
				a.Codebegin=nextstat;
				nextstat++;
				strcpy(willprint[gtnum],"goto ");
				judgegt[gtnum]=0;
...
void printtofile(FILE * fp)
{
	...
	{
		if(gt[i]!=-1)
			fprintf(fp,"%d %s%d",i+100,willprint[i],gt[i]+100);
		else
			fprintf(fp,"%d %s-",i+100,willprint[i]);
		if(judgegt[i]==0)
			fprintf(fp,"   未定");
		fprintf(fp,"\n");
	}
...
void Analysis(struct produce *P, int ***analysistable)
{
	...
	char begin[2]={"#"},empty[2]={"0"},rop[4]={"rop"};
	char word[MAXSYMBOLLENGTH];

	char idtable[MAXNUMBER][MAXSYMBOLLENGTH];
	int nnumber,inumber,n,number=0,nextstat=0;	//nnumber当前表中的符号个数，inumber初始表中的个数（不算扩充的符号）,number扩充符号个数

	int loend=location(P->symbol,begin,P->numberOFsym);
	int loempty=location(P->symbol,empty,P->numberOFsym);

	struct return4 re[3],a;
	loid=location(P->symbol,ID,P->numberOFsym);
	lr=location(P->symbol,rop,P->numberOFsym);
	FILE *fp;
	...
	while(1)
	{
		...
		{
			r=ropfind(word);
			if(lookup(word,idtable,inumber)!=ERROR)
				lo=loid;
			else if(r!=ERROR)
				lo=lr;
			else
			...
			if(analysistable[stackofstatus[top]][lo][1]==GUIYUE)
			{
				...
				if(P->produceR[analysistable[stackofstatus[top]][lo][0]][0]!=loempty)		//如果是空产生式，不做处理
				{
					...
					for(i=0;i<j;i++)		//出栈
					{
						...
						{
							re[n].place=stackofsymbol[top].place;
							re[n].True=stackofsymbol[top].True;
							re[n].False=stackofsymbol[top].False;
							re[n++].Codebegin=stackofsymbol[top].Codebegin;
						}
						if(stackofsymbol[top].rop!=ERROR)
							ROP=stackofsymbol[top].rop;
						top--;
					}
				}
				else
				...
				a=ProductionFunction(k,idtable,nextstat,re,ROP+7);
				stackofsymbol[top].place=a.place;
				stackofsymbol[top].True=a.True;
				stackofsymbol[top].False=a.False;
				stackofsymbol[top].Codebegin=a.Codebegin;
				stackofsymbol[top].rop=r;
			...
			else
			{
				...
				for(i1=0,mark=0;i1<symbolnum;i1++)
				{
