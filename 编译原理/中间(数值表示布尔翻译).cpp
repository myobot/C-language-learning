...
int ProductionFunction(FILE *fp,int Pnum,char idtable[][MAXSYMBOLLENGTH],int &nnumber,int &number,int &nextstat,int p[3],int rop)	//p符号位置
{
	switch(Pnum)
	{
		case 1: p[2]=AdditionTemp(idtable,nnumber,number);
				number++;nextstat++;
				fprintf(fp,"(%d) %s = %s or %s\n",nextstat,idtable[p[2]],idtable[p[1]],idtable[p[0]]);
				return p[2];
				
		case 2: p[2]=AdditionTemp(idtable,nnumber,number);
				number++;nextstat++;
				fprintf(fp,"(%d) %s = %s and %s\n",nextstat,idtable[p[2]],idtable[p[1]],idtable[p[0]]);
				return p[2];

		case 3:	p[2]=AdditionTemp(idtable,nnumber,number);
				number++;nextstat++;
				fprintf(fp,"(%d) %s = not %s\n",nextstat,idtable[p[2]],idtable[p[0]]);
				return p[2];

		case 4:	return p[0];

		case 5:	p[2]=AdditionTemp(idtable,nnumber,number);
				number++;nextstat++;
				fprintf(fp,"(%d) if %s %s %s goto (%d)\n",nextstat,idtable[p[1]],symbol[rop],idtable[p[0]],nextstat+3);
				nextstat++;
				fprintf(fp,"(%d) %s=0\n",nextstat,idtable[p[2]]);
				nextstat++;
				fprintf(fp,"(%d) goto (%d)\n",nextstat,nextstat+2);
				nextstat++;
				fprintf(fp,"(%d) %s=1\n",nextstat,idtable[p[2]]);
		...
		case 6: p[2]=AdditionTemp(idtable,nnumber,number);
				number++;nextstat++;
				fprintf(fp,"(%d) %s=1\n",nextstat,idtable[p[2]]);
				return p[2];

		case 7:	p[2]=AdditionTemp(idtable,nnumber,number);
				number++;nextstat++;
				fprintf(fp,"(%d) %s=0\n",nextstat,idtable[p[2]]);
				return p[2];
		case 8:	return p[0];
...
void Analysis(struct produce *P, int ***analysistable)
{
	...
	int nnumber,inumber,p[3],n,number=0,nextstat=0;	//nnumber当前表中的符号个数，inumber初始表中的个数（不算扩充的符号）,number扩充符号个数
	...
	while(1)
	{
		if(STATUS[stackofstatus[top]].mark==ENDSTATUS)		
		{
			...
			if(analysistable[stackofstatus[top]][lo][1]==GUIYUE)
			{
				...
				stackofsymbol[top].place=ProductionFunction(fp,k,idtable,nnumber,number,nextstat,p,ROP+5);
