...
int ProductionFunction(FILE *fp,int Pnum,char idtable[][MAXSYMBOLLENGTH],int &nnumber,int &number,int p[3])	//p符号位置
{
	switch(Pnum)
	{
		...
		case 4:	p[2]=AdditionTemp(idtable,nnumber,number);
				number++;
				emit(fp,p[2],p[1],p[0],4,idtable,nnumber);return p[2];
		case 5:	p[2]=AdditionTemp(idtable,nnumber,number);
				number++;
				emit(fp,p[2],p[1],p[0],5,idtable,nnumber);return p[2];
		case 6: p[2]=AdditionTemp(idtable,nnumber,number);
				number++;
				emit(fp,p[2],p[0],0,6,idtable,nnumber);return p[2];
		case 7:	return p[0];
		case 8:	return p[0];
		case 9:	return p[0];
		case 10:return p[0];
